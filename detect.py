            ...
            for i, det in enumerate(pred):  # detections per image
                ...
                if len(det):
                    ...
                    for *xyxy, conf, cls in reversed(det):
                        xCenter = ((xyxy[0] + xyxy[2]) / 2).item()
                        yCenter = ((xyxy[1] + xyxy[3]) / 2).item()
                        # print(f"({xyxy[0]}, {xyxy[1]})({xyxy[2]}, {xyxy[3]}) - {cls}; {names[int(cls)]}")
                        detected_sectors.append([(xCenter, yCenter), names[int(cls)]])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--segment', action='store_true', help='save coordinates for segmentation')
